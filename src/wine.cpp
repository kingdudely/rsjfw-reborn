...
namespace fs = std::filesystem;
 ...
 WineRunner::WineRunner(std::shared_ptr<Prefix> prefix,
    ...
    LOG_DEBUG("WineRunner created with bin dir: %s", wineBinDir.c_str());
}
...
bool WineRunner::runWine(const std::string &exe,
  ...
  if (prefix_) {
    ...
    LOG_DEBUG("Running Wine task '%s': %s", taskName.c_str(), exe.c_str());
    return prefix_->wine(
        exe, args,
...
bool WineRunner::configure(ProgressCb cb) {
  ...
  if (!cfg.enableWebView2) {
    if (fs::exists(appDir)) {
      if (cb) cb(0.12f, "cleaning up webview2...");
  ...
  if (fs::exists(marker)) {
    LOG_DEBUG("Prefix already provisioned (marker found)");
    ...
    for (size_t i = 0; i < fonts.size(); i++) {
        ...
        float fontSlice = slice / (float)fonts.size() * 0.5f;
        ...
        if (fs::exists(fexpand)) {
                for (const auto &entry : fs::directory_iterator(fexpand)) {
                    if (entry.path().extension() == ".TTF" || entry.path().extension() == ".ttf") {
                        ...
                        LOG_DEBUG("Installing font file: %s", fname.c_str());
                        fs::copy_file(entry.path(), fontsDir / fname, fs::copy_options::overwrite_existing);
                    }
                }
            }
            fs::remove_all(fexpand);
    ...
    if (cfg.enableWebView2) {
      ...
      prefix_->registryAdd("HKLM\\SOFTWARE\\WOW6432Node\\Microsoft\\EdgeUpdate\\Clients\\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}", "pv", "143.0.3650.139", "REG_SZ");
      prefix_->registryAdd("HKLM\\SOFTWARE\\WOW6432Node\\Microsoft\\EdgeUpdate\\Clients\\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}", "location", "C:\\Program Files (x86)\\Microsoft\\EdgeWebView\\Application", "REG_SZ");
      prefix_->registryAdd("HKCU\\Software\\Wine\\AppDefaults\\msedgewebview2.exe", "Version", "win7", "REG_SZ");
...
std::string WineRunner::resolveWindowsPath(const std::string &unixPath) {
    fs::path p(unixPath);
...
std::map<std::string, std::string> WineRunner::getBaseEnv() {
    auto res = Runner::getBaseEnv();
    auto &cfg = Config::instance().getGeneral();
    auto &pm = PathManager::instance();

    res["WINEPREFIX"] = pm.prefix().string();
    res["WINEARCH"] = "win64";
