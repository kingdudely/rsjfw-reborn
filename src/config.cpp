...
nlohmann::json Config::getClientAppSettings() const {
    json settings;
    for (const auto& [k, v] : fflags_) settings[k] = v;

    if (general_.renderer == "Vulkan") settings["FFlagDebugGraphicsPreferVulkan"] = true;
    else if (general_.renderer == "OpenGL") settings["FFlagDebugGraphicsPreferOpenGL"] = true;
    else if (general_.renderer == "D3D11") settings["FFlagDebugGraphicsPreferD3D11"] = true;

    if (general_.targetFps > 0) settings["DFIntTaskSchedulerTargetFps"] = (int)general_.targetFps;

    if (general_.lightingTechnology == "Future") settings["FFlagDebugForceFutureIsBrightPhase3"] = true;
