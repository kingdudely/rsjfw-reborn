...
bool PresetManager::importPreset(const fs::path &path) {
  ...
  try {
    std::ifstream f(path);
    json j = json::parse(f);
    if (!j.contains("general") && !j.contains("fflags"))
      return false;
...
bool PresetManager::exportPreset(const std::string &name,
                                 const fs::path &dest) {
...
json PresetManager::sanitizeForExport(const json &inJ) {
  ...
  auto sanitizeSource = [](json &s) {
    if (s.contains("useCustomRoot") && s["useCustomRoot"] == true) {
      ...
      bool isSystem = (p.find("/usr/") == 0 || p.find("/opt/") == 0 ||
                       p.find("/bin/") == 0);
      if (!isSystem) {
  ...
  if (j["general"].contains("wineSource"))
    sanitizeSource(j["general"]["wineSource"]);
  if (j["general"].contains("protonSource"))
    sanitizeSource(j["general"]["protonSource"]);
