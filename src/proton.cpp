...
ProtonRunner::ProtonRunner(std::string protonRoot)
  ...
  compatDataPath_ = pm.proton().string();
...
cmd::CmdResult ProtonRunner::runStudio(const std::string &versionGuid,
  ...
  LOG_DEBUG("Killing leftover wineserver before launch...");
  prefix_->kill();
...
std::map<std::string, std::string> ProtonRunner::getBaseEnv() {
  ...
  res["STEAM_COMPAT_DATA_PATH"] = pm.proton().string();
  res["STEAM_COMPAT_CLIENT_INSTALL_PATH"] =
      (fs::path(getenv("HOME")) / ".local/share/Steam").string();
  res["WINEPREFIX"] =
      (fs::path(res["STEAM_COMPAT_DATA_PATH"]) / "pfx").string();
  return res;
}
